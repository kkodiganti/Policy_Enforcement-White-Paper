...
- **Byline and disclaimer are required together.** Any published version of the white paper must carry the full byline — name, professional-society credentials line (*Fellow IETE, Fellow IAENG, Fellow SCRS, Senior Member IEEE, Member ACM*, ordered Fellow → Senior Member → Member), job title/employer line (*Senior Lead Software Engineer, Capital One*) — and the disclaimer that views are the author's own and don't represent Capital One, in the title block, the Contact Information section, and any excerpt used elsewhere (e.g., a LinkedIn post or an SSRN abstract page). If a new credential is earned or a grade changes, update all three places, not just one.
