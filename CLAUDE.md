...
- `SSRN-SUBMISSION.md` — prep sheet for the SSRN submission form: keywords, verified SSRN classifications, license recommendation (CC BY), and draft Declaration of Interest / Funder / Ethics Approval statements. Not part of the paper itself. The Declaration of Interest has two undecided variants (name Capital One as employer, or keep it anonymized to match the byline) — resolve before actually submitting.
- `ACADEMIA-EDU-SUBMISSION.md` — prep sheet for uploading to academia.edu, treated as fully independent of SSRN (no shared fields, no cross-referencing — an earlier draft incorrectly assumed a dependency and was corrected). Publication Name recommendation is to leave it blank or use "Independent Publication," since this is self-published work with no journal venue. Also has a 20-item Research Interests list that assumes a profile spanning this author's other white papers (Human-Identification, Data-Quality-Pitfalls) too, not just this repo — flagged in the file itself as an assumption to confirm.
